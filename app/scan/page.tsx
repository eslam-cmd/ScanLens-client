...
const buildGuestScanData = (count: number, now = new Date()) => ({
  count,
  date: now.toDateString(),
  resetAt: getNextResetTimestamp(now),
});
...
const getGuestScanData = () => {
  try {
    ...
    if (data) {
      ...
      if (parsed.resetAt && parsed.resetAt > now.getTime()) {
        return {
      ...
      return buildGuestScanData(0, now);
    }
  } catch {
    // تجاهل الأخطاء
  }

  return buildGuestScanData(0);
};
...
const saveGuestScanData = (count: number) => {
  try {
    ...
    localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify(buildGuestScanData(count, now)),
    );
  } catch {
    // تجاهل الأخطاء
  }
...
function ScanPageContent({ user: initialUser }: { user?: any }) {
  ...
  const [limitResetAt, setLimitResetAt] = useState<number>(() =>
    getNextResetTimestamp(new Date()),
  );
  const [resetCountdown, setResetCountdown] = useState(() => {
    const now = Date.now();
    const targetResetAt =
      guestScanData.resetAt || getNextResetTimestamp(new Date());
    const remaining = Math.max(0, targetResetAt - now);
  ...
  // ✅ حساب عدد الفحوصات التي تمت اليوم فقط
  const getTodayScansUsed = useCallback(() => {
    const todayStart = new Date();
    todayStart.setHours(0, 0, 0, 0);

    return (
      scanHistory?.filter((item) => {
        const createdAt = new Date(item?.createdAt || item?.completedAt || 0);
  ...
  const getScanLimitStatus = useCallback(() => {
    if (isLoggedIn) {
      ...
      const scansUsed = getTodayScansUsed();
  ...
  // ✅ تحديث عداد إعادة المحاولات اليومية في الوقت الحقيقي
  useEffect(() => {
    if (!isLoggedIn) {
      setLimitResetAt(
        guestScanData.resetAt || getNextResetTimestamp(new Date()),
    ...
    setLimitResetAt(getNextResetTimestamp(new Date()));
  }, [isLoggedIn, guestScanData.resetAt]);

  useEffect(() => {
    const updateResetCountdown = () => {
      const now = Date.now();
      const remaining = Math.max(0, limitResetAt - now);
      const totalSeconds = Math.max(0, Math.ceil(remaining / 1000));
      ...
      if (remaining <= 0) {
        ...
        const resetData = buildGuestScanData(0, new Date());
        setGuestScanData(resetData);
        saveGuestScanData(0);
        setLimitResetAt(resetData.resetAt);
      }
    };
    ...
    return () => clearInterval(interval);
  }, [limitResetAt, isLoggedIn, fetchScanHistory]);
          ...
          <div className="p-6 rounded-2xl bg-amber-500/10 border-2 border-amber-500/30 text-center space-y-4">
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="flex items-center gap-2 text-xs text-slate-400 bg-slate-900/50 px-4 py-2 rounded-lg">
                ...
                <span>
                  Limit resets in{" "}
                  {`${String(resetCountdown.hours).padStart(2, "0")}:${String(
                    resetCountdown.minutes,
                  ).padStart(
                    2,
                    "0",
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">
                  {guestScanCount >= MAX_GUEST_SCANS
                    ...
                    : `Daily reset in ${String(resetCountdown.hours).padStart(2, "0")}:${String(
                        resetCountdown.minutes,
                      ).padStart(2, "0")}:${String(
